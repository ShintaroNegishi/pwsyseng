...
def solve(
    ...
    **混合整数計画では :attr:`Solution.duals` は空の辞書になる。**
    これは実装の手抜きではない。混合整数計画の整数最適解には、線形計画と
    同じ意味で一意に解釈できる影価格を一般には定義できない。連続緩和には
    双対変数があるが、それを元の整数問題の限界費用としてそのまま解釈しては
    ならない。CBC が探索中の緩和問題について返す双対も、整数最適解の
    影価格ではない。意味のある値が欲しいときは、得られた整数解を固定して線形計画に落とし直してから双対を取る
