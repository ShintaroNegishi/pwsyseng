...
# %% [markdown]
# 6 件中 4 件が不合格で、うち **2 件は事故後潮流が収束すらしません**。第 03 回でやった
# とおり、収束しない原因は解の不存在とは限りません（初期値・反復上限でも起きます）。
# ただ、どちらであっても **この定式化では事故後の定常状態を確認できない**ので、
# 安全側に不合格として扱います。存在しないことの証明ではありません。
#
